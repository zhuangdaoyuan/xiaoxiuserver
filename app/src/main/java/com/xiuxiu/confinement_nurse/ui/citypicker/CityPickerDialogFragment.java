package com.xiuxiu.confinement_nurse.ui.citypicker;
...
import com.xiuxiu.confinement_nurse.ui.citypicker.adapter.CityListAdapter;
import com.xiuxiu.confinement_nurse.ui.citypicker.adapter.InnerListener;
import com.xiuxiu.confinement_nurse.ui.citypicker.adapter.OnPickListener;
import com.xiuxiu.confinement_nurse.ui.citypicker.adapter.decoration.SectionItemDecoration;
...
import com.xiuxiu.confinement_nurse.ui.citypicker.model.HotCity;
...
import com.xiuxiu.location.manager.view.SideIndexBars;
import com.xiuxiu.location.R;
...
public class CityPickerDialogFragment extends DialogFragment implements TextWatcher,
        View.OnClickListener, SideIndexBars.OnIndexTouchedChangedListener, InnerListener, AreaContract.IView {
    private View mContentView;
    private RecyclerView mRecyclerView;
    private View mEmptyView;
    private TextView mOverlayTextView;
    private SideIndexBars mIndexBar;
    ...
    private void initData() {
        ...
        if (mHotCities == null || mHotCities.isEmpty()) {
            ...
            mHotCities.add(new HotCity("北京", "北京", "110000"));
            mHotCities.add(new HotCity("上海", "上海", "310000"));
            mHotCities.add(new HotCity("广州", "广东", "440100"));
            mHotCities.add(new HotCity("深圳", "广东", "440300"));
            mHotCities.add(new HotCity("天津", "天津", "120000"));
    ...
    public void onRequestCitys(CityVm locationVm) {
        List<City> items1 = locationVm.getItems();
        if (items1 == null) {
            dbManager = new DBManager(getActivity());
            mAllCities = dbManager.getAllCities();
        } else {
            List<City> cities=new ArrayList<>();
            for (City city:items1){
                if (city.getChildren()!=null) {
                    cities.addAll(city.getChildren());
                }else{
                    cities.add(city);
                }
            }
            Collections.sort(cities, new DBManager.CityComparator());
            mAllCities = cities;
        }
        //初始化定位城市，默认为空时会自动回调定位
        if (mLocatedCity == null) {
        ...
        mResults = mAllCities;
        ((SectionItemDecoration) (mRecyclerView.getItemDecorationAt(0))).setData(mResults);
        mAdapter. updateData(mAllCities);
    }

    @Override
    public void onRequestLocations(LocationVm locationVm) {
