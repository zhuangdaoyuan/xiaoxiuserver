...
import com.lxj.xpopupext.popup.CollectionUtil;
...
public class WheelOptions<T> {
            ...
            public void onItemSelected(int index) {
                ...
                if (mOptions2Items == null) {//只有1级联动数据
                    ...
                    if (!isRestoreItem) {
                        ...
                        opt2Select = Math.max(opt2Select, 0);
                    }
            ...
            public void onItemSelected(int index) {
                if (mOptions3Items != null) {
                    ...
                    if (!isRestoreItem) {
                        ...
                        List<List<T>> safe = CollectionUtil.getSafe(mOptions3Items, opt1Select, null);
                        if (safe == null) {
                            opt3 = 0;
                        } else {
                            List<T> safe1 = CollectionUtil.getSafe(safe, index, null);
                            if (safe1 != null) {
                                int size = safe1.size();
                                opt3 = Math.min(wv_option3.getCurrentItem(), size - 1);
                                opt3 = Math.max(opt3, 0);
                            } else {
                                opt3 = 0;
                            }
                        }
                    ...
                    try {
                        List<T> ts = mOptions3Items.get(wv_option1.getCurrentItem()).get(index);
                        wv_option3.setAdapter(new ArrayWheelAdapter(ts));
                    } catch (Exception e) {
    ...
    public int[] getCurrentItems() {
        ...
        if (mOptions3Items != null && mOptions3Items.size() > 0) {//非空判断
            ...
            List<T> safe = CollectionUtil.getSafe(lists, currentItems[1], null);
            currentItems[2] = wv_option3.getCurrentItem() > (safe == null ? 0 : safe.size() - 1) ? 0 : wv_option3.getCurrentItem();
        } else {
